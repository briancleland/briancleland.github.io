	...
	function findConstituencyInfo(year, filename) {
		var request = new XMLHttpRequest();
		var path = './'+year+'/NI/' + filename;
		console.log(path);
		request.onreadystatechange = function() {
	...
	constituencyinfo.update = function () {
		findConstituencyInfo(checkedYear, 'all-constituencies-count-info.json');
		var constituency = getObjects(constituenciesCount, 'Constituency_Number', constituency_id);
		console.log(constituency);
		this.innerHTML += '<b>Electorate:</b> ' + numberWithCommas(constituency[0].Total_Electorate) + '</br><b>Voted:</b> ' + numberWithCommas(constituency[0].Total_Poll) + '</br><b>Turnout:</b> ' + ((constituency[0].Total_Poll/constituency[0].Total_Electorate) *100).toFixed(2) + '%';
	};
